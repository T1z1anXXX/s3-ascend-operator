...
private:
    ...
    __aicore__ inline void Compute(int32_t progress, uint32_t length) {
        ...
        if constexpr (std::is_same_v<T, int8_t>) {
            ...
            Compare(bits, float_x1, float_x2, CMPMODE::NE, length);
            ...
            if constexpr (std::is_same_v<T, int32_t>) {
                ...
                Sub(x1, x1, x2, length);
                Mins(x1, x1, (int32_t)POSITIVE_ONE_I32, length);
                Maxs(x1, x1, (int32_t)NEGATIVE_ONE_I32, length);
                Abs(x1, x1, length);
                Cast(float_x1, x1, RoundMode::CAST_NONE, length);
                Cast(result, float_x1, RoundMode::CAST_NONE, length);
            }
            else{ //half float
                Compare(bits, x1, x2, CMPMODE::NE, length);
                Select(result, bits, one, half(0), SELMODE::VSEL_TENSOR_SCALAR_MODE, length);
            }
        }
...
template<typename TYPE_X1, typename TYPE_X2, typename TYPE_Y> class KernelNotEqualBroadcast{
    using T = TYPE_X1;
public:
    __aicore__ inline KernelNotEqualBroadcast() {}
    __aicore__ inline void Init(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, uint32_t totalLength, 
                            uint32_t ALIGN_NUM, uint32_t block_size, uint32_t core_size, uint32_t core_remain, 
    ...
    __aicore__ inline void Process() {
        ...
        for(int j = 0; j < this->y_sumndarray[dim]; j++){
            ...
            if constexpr (std::is_same_v<T, int8_t>) {
                ...
                Compare(bits, float_x1, float_x2, CMPMODE::NE, 1);
                Select(result, bits, one, half(0), SELMODE::VSEL_TENSOR_SCALAR_MODE, 1);
            }
            else {
                if constexpr (std::is_same_v<T, int32_t>) {
                    Duplicate(result, (half)(1.0), 1);
                    auto float_x1 = B_x1.Get<float>();
                    Sub(x1Local, x1Local, x2Local, 1);
                    Mins(x1Local, x1Local, (int32_t)POSITIVE_ONE_I32, 1);
                    Maxs(x1Local, x1Local, (int32_t)NEGATIVE_ONE_I32, 1);
                    Abs(x1Local, x1Local, 1);
                    Cast(float_x1, x1Local, RoundMode::CAST_NONE, 1);
                    Cast(result, float_x1, RoundMode::CAST_NONE, 1);
                }
                else{ //half float
                    Compare(bits, x1Local, x2Local, CMPMODE::NE, 1);
                    Select(result, bits, one, half(0), SELMODE::VSEL_TENSOR_SCALAR_MODE, 1);
                }
            }
            Cast(inty, result, RoundMode::CAST_ROUND, 1);
            // yLocal.SetValue(0, (TYPE_Y)(1));
...
extern "C" __global__ __aicore__ void not_equal(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    if(TILING_KEY_IS(1)){
        ...
        KernelNotEqualBroadcast<DTYPE_X1, DTYPE_X2, DTYPE_Y> op;
        op.Init(x1, x2, y, tiling_data.totalLength, tiling_data.ALIGN_NUM, tiling_data.block_size, tiling_data.core_size, tiling_data.core_remain, 
